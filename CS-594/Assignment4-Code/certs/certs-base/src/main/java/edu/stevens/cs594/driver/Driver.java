...
public class Driver<Command extends Enum<Command>, Option extends Enum<Option>> {
	...
	private static final String CONFIG_PROPERTIES = "driver";

	private static final String PROMPT_KEY = "interactive.prompt";
	
	// Prompts for the dialog for a read-eval-print loop
	private static ResourceBundle config;
	
	private static final String WARNING_INTERACTIVE = "This app must be run in interactive mode.";

	private static final String Prompt;
	
	static {
		config = ResourceBundle.getBundle(CONFIG_PROPERTIES);
		if (config == null) {
			throw new IllegalStateException("Missing resource bundle: "+ CONFIG_PROPERTIES);
		}
		Prompt = config.getString(PROMPT_KEY) + "> ";
	}
	...
	private String readLine(String prompt) throws IOException {
		...
		if (console == null) {
			throw new IOException(WARNING_INTERACTIVE);
		}
