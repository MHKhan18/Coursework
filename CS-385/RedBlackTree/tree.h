...
class Tree {
public:
    // virtual keyword helps polymorphism
    virtual ~Tree() { };
    virtual std::string to_ascii_drawing() = 0;  
    virtual int height() const = 0;  // const : can't change member variables
    virtual size_t size() const = 0;  // =0 -> pure virtual function : the function must be overwritten in a derived class
