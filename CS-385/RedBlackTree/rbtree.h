...
/*. A forward declaration tells the compiler about the existence of an entity before 
actually defining the entity. Forward declarations can also be used with other entity in 
C++, such as functions, variables and user-defined types. */
// Forward declaration:
 ...
 */
class tree_exception: public std::exception {   // inheritance in c++
public:
...
public:
    ...
    void insert_elements(std::vector< std::pair<K, V> > &elements) {
        for (size_t i = 0, len = elements.size(); i < len; ++i) {
            try {
                ...
                std::cerr << "Warning: " << te.what() << std::endl;  // have to raise errors in the insert() method
            }
        }
    }
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        // throw exception for duplicates
        if ( find(key) != end() ){
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        // TODO
        // do post order traversal and delete
        if ( n == NULL ){
            return ;
        }
        else{
    ...
    void insert_fixup(Node<K, V> *z) {
        ...
        while ( z->parent != NULL && z->parent->color == RED ){         
            if (  z->parent == z->parent->parent->left ){   // grand parent must exist by property
                Node<K,V> *y = z->parent->parent->right; // uncle
                if ( y != NULL && y->color == RED ){  // gotta check for uncle being null!!
    ...
    int diameter(Node<K, V> *node) const {
        ...
        i) the diameter of T's left subtree
        ii) the diameter of T's right subtree
        iii) the longest path between leaves that goes through the root of T (this can be computed from the heights of the subtrees of T) */
        if ( node == NULL ){
            return 0;
        }
        int left = diameter(node->left);
        int right = diameter(node->right);
        int inc_root = 2 + height(node->left) + height(node->right);
        return std::max( inc_root, std::max(left,right));
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        level-=1;
        return width(node->left, level) + width(node->right, level); // warning flags won't let decrease level inside function call!!
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        // TODO
        size_t ht = static_cast<size_t>(height(node));

        size_t res = 0; 
        for ( size_t lv=level; lv <= ht; ++lv ){
            res += width(node,lv)*lv ;
        }
