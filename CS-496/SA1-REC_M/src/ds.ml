...
let rec get_triple: Ast.decs -> string -> (string*string*Ast.expr) option  =
  fun decs id ->
    match decs with 
    | [] -> None 
    | (v, par, body) :: t ->
        if v = id then Some (v, par, body)
        else get_triple t id 
...
*)
  | ExtendEnvRec(decs,tail) ->
      let target_triple = get_triple decs id  in 
      match target_triple with 
      | None -> apply_env id tail 
      | Some (_, par, body) -> Ok (ProcVal (par,body,env))
