...
(* super_class_name * fields_list * methods_env *)
...
let rec lookup_class : string -> class_env -> class_decl ea_result = 
  fun c_name c_env ->
    match c_env with 
    | (name,dec) :: t ->
        if name = c_name
        then return dec 
        else lookup_class c_name t 
    | [] -> error "lookup_class: class c_name not found"
    
let rec new_env' : string list -> env = fun fs ->
  match fs with
...
let rec lookup_method_helper : string -> method_env ->  method_decl option = 
  fun m_name m_env ->
  match m_env with 
  | (m_found , m_decl)::t ->
      if m_found = m_name
      then Some ( m_decl)
      else lookup_method_helper m_name t 
  | [] -> None 

    
(* c_name * m_name * class_env *)
let rec lookup_method : string -> string -> class_env ->method_decl option = 
    ...
    | (name,dec) :: t ->
        ( if name = c_name
          then 
            (match dec with 
              | (_, _, met_envs ) ->
                  ( lookup_method_helper m_name met_envs ))
          
          else lookup_method c_name m_name t )
    | [] -> None )
    ...
    sequence (List.map eval_expr es)  >>= fun args ->
      lookup_class c_name (!g_class_env) >>= fun ( _, fields , methods ) -> 
              new_env fields >>= fun env ->  
                let obj = ObjectVal(c_name ,env) in 
                  (match (List.assoc_opt "initialize" methods)  with 
                    | Some m -> (apply_method "initialize" obj  args  m) >>= fun _ -> return obj 
                    | None -> return obj )

| Send(e,m_name,es) ->
    eval_expr e >>= obj_of_objectVal >>= fun t ->
      sequence (List.map eval_expr es)  >>= fun args ->
      ( match t with 
        | (c_name, en ) -> 
            ( match (lookup_method c_name m_name !g_class_env) with
              | None -> error "Method not found"
              | Some m -> apply_method m_name (ObjectVal(c_name,en)) args  m ) )
